...
def update(frame):
    ...
    bars = plt.barh(sorted_scores.index[::-1], sorted_scores.values[::-1], color=[colors[jugador] for jugador in sorted_scores.index], alpha=0.2)  # Establecer transparencia para todas las barras
    
    # Resaltar jugadores especificados
    highlighted_players = ["Jugador 20"]  # Puedes cambiar esto por una lista de jugadores que desees resaltar
    if highlighted_players:
        for bar, jugador in zip(bars, sorted_scores.index):
    ...
    plt.gca().tick_params(axis='y', which='both', left=False, labelleft=False)  # Eliminar los ticks y etiquetas del eje y
    plt.tight_layout()
